  ...
  if (listProducts.length > 0) {
    listProducts.map(product => {
      newListProductSchema.push({
        ...
        "CodeProjetCapex": product.CapexProjectSelectedID,
...
export function getOrderFilter(filter1, filter2, search) {
  const isF1All = filter1 === "TOUS";
  const isF2All = filter2 === "TOUS";
  const hasSearch = search && search.trim() !== "";

  if (isF1All && isF2All && !hasSearch) return 1; // nothing selected
  if (isF1All && isF2All && hasSearch) return 2;  // only search
  if (isF1All && !isF2All && !hasSearch) return 3; // only filter2
  if (isF1All && !isF2All && hasSearch) return 4;  // filter2 + search
  if (!isF1All && isF2All && !hasSearch) return 5; // only filter1
  if (!isF1All && isF2All && hasSearch) return 6;  // filter1 + search
  if (!isF1All && !isF2All && !hasSearch) return 7; // filter1 + filter2
  return 8; // filter1 + filter2 + search
}
...
export function getApprouverListOrder(subfamily, beneficiaire, listSubfamilysApprouvers) {
  console.log(subfamily, beneficiaire, listSubfamilysApprouvers)
...
export function getArticles(articles) {
  return articles.reduce((acc, article, index) => {
    ...
    acc[`Description technique Article ${index + 1}`] = article.DescriptionTechnique || "";
    acc[`Réference article ${index + 1}`] = article.ArticleREF || "";
    acc[`Projet Capex article ${index + 1}`] = article.CapexProjectSelected || "";
...
export function exportJsonToExcel(jsonData, fileName) {
  ...
  jsonData.map(demande => {
    ...
    const articlesRows = articles.map(article => ({
      ...
      "Reference Demande": demande.ReferenceDemande?.toString() || "",
      "Budget Demande": demande.budgetSelected || "",
