...
export default class DashboardDemandeCloturees extends React.Component<IDashboardDemandeClotureesProps, {}> {
  ...
  private clearFilterButton = () => {
    this.setState({ IdIntranetFilter: '', referenceErpFilter: '', StatusFilter: '', dateDebutFilter: '', dateFinFilter: '', listDemandeData: [], getDataClicked: false });
  }
    ...
    if (IdIntranetFilter.length > 0 || referenceErpFilter.length > 0) {
      ...
      if (IdIntranetFilter.length === 0) {
        filteredData = listDemandeData.filter((item: any) => {
          return item.RéfRequestERP.toLowerCase().includes(referenceErpFilter);
        });
        ...
        filteredData = listDemandeData.filter((item: any) => {
          return item.IdRequestIntranet.toLowerCase().includes(IdIntranetFilter.toLowerCase()) && item.RéfRequestERP.toLowerCase().includes(referenceErpFilter);
        });
      }
    } else {
