...
import { DefaultPalette, Dropdown, IDropdownStyles, AnimationClassNames, DatePicker, mergeStyleSets, ComboBox, TextField } from 'office-ui-fabric-react';
...
export default class DemandeurDashboard extends React.Component<IDemandeurDashboardProps, {}> {
  ...
  private clearFilterButton = () => {
    this.setState({ StatusFilter: '', FamilleFilter: '', descTechFilter: '' });
  }
  ...
  public render(): React.ReactElement<IDemandeurDashboardProps> {
    ...
    const { currentPage, itemsPerPage, listDemandeData, FamilleFilter, StatusFilter, descTechFilter } = this.state;
    var filteredData
    if (FamilleFilter.length > 0 || StatusFilter.length > 0 || descTechFilter.length > 0) {
      console.log(FamilleFilter)
      console.log(StatusFilter)
      const orderFilter = getOrderFilter(FamilleFilter, StatusFilter, descTechFilter);
      ...
      if (orderFilter === 1) {
        ...
        // Only Famille
        filteredData = listDemandeData.filter((item: any) =>
          item.FamilleProduit.toLowerCase().includes(FamilleFilter.toLowerCase())
        );
      ...
      <div className={styles.demandeurDashboard}>
        ...
        {(listDemandeData.length > 0 && !this.state.showSpinner) &&
          <div id="spListContainer">
            <table style={{ borderCollapse: "collapse", width: "117%" }}>
              <tr><th className={styles.textCenter}>#</th> <th>№</th><th>Famille demande</th><th>Centre de gestion</th><th>Date de la Demande</th><th>Statut de la demande</th><th>Date d'approbation final</th>{this.state.checkActionCurrentUser && <th>Action</th>}<th>Desc/Tech</th><th>Détail</th></tr>
              {currentItems.map((demande: any, index: any) =>
                <tr>
                  ...
                  <td style={{ textAlign: "center" }}>
                    <span className={styles.icon} style={{ cursor: "pointer" }} title="afficher directement la description technique">
                      <svg
