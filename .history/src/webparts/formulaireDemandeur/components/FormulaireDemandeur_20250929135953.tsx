...
export default class FormulaireDemandeur extends React.Component<IFormulaireDemandeurProps, {}> {
  ...
  private handleChangeCapexProjectDropdown = async (event: any, index: any) => {
    ...
    console.log("TEST CAPEX", updatedFormData[index - 1].SousFamilleSelected[0]?.key, event.key, this.state.BudgetID, updatedFormData[index - 1].ArticleSelected[0]?.key, event.CodeProjetCapex)

    const item = await getProductCapex(updatedFormData[index - 1].SousFamilleSelected[0]?.key, event.key, this.state.BudgetID, updatedFormData[index - 1].ArticleSelected[0]?.key, event.CodeProjetCapex);
    console.log("ITEM CAPEX", item)
