...
export default class UpdateDemandeAchat extends React.Component<IUpdateDemandeAchatProps, {}> {
  ...
  private handleChangeArticleDropdown = async (event: any, index: any) => {
    ...
    if (!this.state.DisabledBenef) {
      ...
      if (this.state.demandeAffectation === "me") {
        ...
        // items = await getProjectCapex(event.Axe, "HEALTH", this.state.BudgetID);
        console.log(items)
      } else {
  ...
  public getCurrentDemandeInfo = async () => {
    ...
    listProduits.map(produit => {
      ...
      if (index === 1) {
        const updatedFormData = [...this.state.formData];
        updatedFormData[index - 1].budgetSelected = [{ "key": demandeData.budgetSelectedID, "text": demandeData.budgetSelected }]
        ...
        updatedFormData[index - 1].ArticleSelected = [
          {
            "key": produit.ArticleREF,
            "text": produit.DescriptionTechnique,
            "Axe": produit.Axe,
            "BudgetAnnualAllocated": produit.BudgetAnnualAllocated,
            "BudgetAnnualRemaining": produit.BudgetAnnualRemaining,
            "BudgetAnnualUsed": produit.BudgetAnnualUsed,
            "LatestPurchasePrice": produit.LatestPurchasePrice,
            "IsCapex": produit.IsCapex
          }
        ],
          updatedFormData[index - 1].BeneficiareSelected = [{ "key": produit.BeneficiaireID, "text": produit.Beneficiaire }]
        updatedFormData[index - 1].projectSelected = [{ "key": produit.CapexProjectSelectedID, "text": produit.CapexProjectSelected }]
        ...
        this.setState({
          ...
          BudgetID: updatedFormData[0].budgetSelected[0].key,
        ...
        var nullObject
        nullObject = {
          budgetSelected: [{ "key": demandeData.budgetSelectedID, "text": demandeData.budgetSelected }],
          ...
          ArticleSelected: [{ "key": produit.ArticleREF, "text": produit.DescriptionTechnique, "Axe": produit.Axe, "BudgetAnnualAllocated": produit.BudgetAnnualAllocated, "BudgetAnnualRemaining": produit.BudgetAnnualRemaining, "BudgetAnnualUsed": produit.BudgetAnnualUsed, "LatestPurchasePrice": produit.LatestPurchasePrice, "IsCapex": produit.IsCapex }],
        ...
        if (!this.state.DisabledBenef) {
          ...
          if (Article.fileName !== null) {
            ...
            ArticleList.push({
              ...
              "CapexProjectSelected": Article.projectSelected?.text || "",
              "CapexProjectSelectedID": Article.projectSelected?.key || "",
              "ArticleFileData": {
  ...
  async componentDidMount() {
    ...
    listProduits.map(async (produit, index) => {
      const updatedFormData = [...this.state.formData];
      console.log(produit)
      const items = await getProduct(produit.SousFamilleID, produit.Beneficiaire, demandeData.budgetSelectedID);
      const capexProjects = await getProjectCapex(produit.Axe, produit.Beneficiaire, demandeData.budgetSelectedID)

      console.log('CAPEX: ', capexProjects)
      const listProjectCapex = capexProjects?.CapexListActiflist?.map(item => ({
        key: item.CodeProjetCapex,
        text: item.NomProjetCapex,
        CodeProjetCapex: item.CodeProjetCapex,
        NomProjetCapex: item.NomProjetCapex,
      ...
      updatedFormData[index].allProjectSelected = listProjectCapex
        ...
        <div className={stylescustom.modifierDemande}>
          <div className={stylescustom.DC}>
            ...
            <SweetAlert2
              ...
              title="Demande des Articles"
              html={`Demande modifiée avec succès. <br><b>Avertissement :</b> dépassement de budget constaté.`}
              imageUrl={img}
              confirmButtonColor="#7D2935"
              onConfirm={() =>
